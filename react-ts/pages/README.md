...
<p>
Change or delete `my-page` example page.
</p>
...
```typescript
import { facturabot_home_path, facturabot_info_path } from './path_pages';
        ...
        {Object.entries(facturabot_menu).map((module, key) => {
                ...
                <ul>
                  {Object.entries(module[1].children).map((m, key) => (
                    <li key={`nav_key_${m[1].path}_${key}`}>
                      <Link to={`${m[1].fullPath}`}>{m[1].label}</Link>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          return (
            <li key={`nav_key_${module[1].path}_${key}`}>
              <Link to={`${module[1].fullPath}`}>{module[1].label}</Link>
            </li>
          );
        })}
      </ul>
